...
describe('TableListPokemonComponent', () => {
  ...
  it('should load the list of Pokémon on init', () => {
    expect(listServiceMock.getPokemonList).toHaveBeenCalledWith(component.limit, component.offset);
    expect(component.pokemons.length).toBe(1);
    expect(component.pokemons[0].name).toBe('BULBASAUR'); // Verifica que el nombre se haya transformado a mayúsculas
  });
  ...
  it('should call getPokemonDetails when a Pokémon is toggled as favorite', () => {
    component.toggleFavorite(0); // Marca el primer Pokémon como favorito
    expect(listServiceMock.getPokemonByIdOrName).toHaveBeenCalledWith('1'); // Verifica que se llamara con el ID correcto
  });
  ...
  it('should open the Pokémon modal when a Pokémon is selected', () => {
    ...
    expect(component.openPokemonModal).toHaveBeenCalledWith(component.selectedPokemon);
  });

  it('should mark all Pokémon as favorites when selectAllFavorites is called', () => {
    component.selectAllFavorites();
    expect(component.pokemons.every(pokemon => pokemon.isFavorite)).toBeTrue();
  });

  it('should navigate to the next page when nextPage is called', () => {
    const initialOffset = component.offset;
    component.totalPokemons = 40; // Simular que hay más Pokémon para paginar
    component.nextPage();
  ...
  it('should navigate to the previous page when previousPage is called', () => {
    component.offset = 20; // Simular que estamos en la segunda página
    const initialOffset = component.offset;
  ...
  it('should show an alert if trying to view favorites when there are none', () => {
    ...
    expect(window.alert).toHaveBeenCalledWith('No hay Pokémon favoritos seleccionados.');
  });
});
